...
```{r echo=FALSE}
last.edit <- "2015-01-27"
x.y <- paste0(R.version$major, ".", R.version$minor)
rw.ver <- paste0("R-", x.y)
xgobi <- paste0('`"d:\\', rw.ver, '\\library\\xgobi\\scripts\\xgobi.bat"')
readme.ver <- paste0('`README.', rw.ver, '`')
gnu32 <- paste0("`", rw.ver, "/src/gnuwin32`")
src <- paste0("`", rw.ver, "/src/library/mypkg/src`")
rw.lib <- paste0("`", rw.ver, "\\library`")
win.version <- paste0("`", rw.ver, "-win.exe`")
```
...
that version.  The main R FAQ can be found at http://CRAN.R-project.org/doc/FAQ/R-FAQ.html.
The information here applies only to recent versions of R for Windows,
(`3.1.0` or later).  It is biased towards users of 64-bit Windows.
...
for a list), navigate to the `bin/windows/base` directory and
collect the file(s) you need.  The current release is distributed as an
installer '`r win.version`' of about 60MB.

There are also links on that page to the `r-patched` and
...
## How do I install R for Windows? {#install-windows}

Current binary versions of R run on Windows XP or later, including on
64-bit versions: [Can I use R on 64-bit Windows?](#64windows). The last version
known to run on Windows 2000 was 2.12.2.

[//]: # (2000 went end-of-life 2010-07-13)
[//]: # (XP SP3 went end of life 2014-04-08)
...
75--110MB of disk space (up to 150MB if ICU was included in the build)
...
account with administrator rights[^footnonadmin] should consider installing into a non-system area (such as
`C:\R`).  Installing to a network share (a filepath starting with
`\\machine\...`) is not supported: such paths will need to mapped
to a network drive.

[^footnonadmin]: Non-administrator accounts will automatically be offered a default installation directory in the user area.

To install use `r win.version`.  Just double-click on the icon
...
([Should I run 32-bit or 64-bit R?](#32-64))
...
space), for example `--sdi --max-mem-size=1G`.  You can also set
environment variables at the end of the Target field, for example
`R_LIBS=p:/myRlib`, and if you want to ensure that menus and
...
(http://jrsoftware.org//ishelp.php) for details.
...
`tcsh` or `bash`. (The command line can be anything you
...
executables are not by default added to the `PATH`.)  People running
from a terminal usually prefer to run `Rterm.exe` and not
`Rgui.exe`.
...
Normally you can do this from the '`Programs and Features`' group in
the Control Panel.  If it does not appear there, run `unins000.exe`
...
copy (say) `R\win-library\3.0` to `R\win-library\3.1` before
...
`--max-mem-size` ([How do I install R for Windows?](#install-windows)) or by
...
`memory.limit` within a running R session.
...
`--max-mem-size` is the smaller of the amount of RAM and 2.5GB.  On
...
`--max-mem-size` is the smaller of the amount of RAM and 3.5GB.
...
Alternatively, start R by double-clicking on a saved `.RData` file
in the directory for the project you want to use, or drag-and-drop a
file with extension `.RData` onto an R shortcut.  In either case,
...
can print help files by `help(fn_name, help_type="PDF")`.
...
You can also set up a batch file using `Rterm.exe`.  A sample
batch file might contain (as one line)

    path_to_R\bin\x64\Rterm.exe --no-restore --no-save < %1 > %1.out 2>&1

The purpose of `2>&1` is to redirect warnings and errors to the
same file as normal output.


## Can I use `r rw.ver` with ESS and Emacs?

Yes.  ESS has for a long time supported R under Windows: it does so by
running `Rterm.exe` without a visible console.

For help with ESS, please send email to ESS-help@stat.ethz.ch,
not the R mailing lists.


## What are HOME and working directories? {#what-home}
...
(typically `C:\Users\username\Documents`).  If that fails, if both
environment variables `HOMEDRIVE` and `HOMEPATH` are set (and
...
## How do I set environment variables? {#set-environment}

Environment variables can be set for `Rgui.exe` and
`Rterm.exe` in three different ways.

1. On the command line as name=value pairs.  For example in the shortcut to
`Rgui` you could have 

        "path_to_R\bin\x64\Rgui.exe" HOME=p:/ R_LIBS=p:/myRlib

1. In an environment file `.Renviron` in the working directory
...
`etc\Renviron.site` and set environmental variables in that file in
...
overridden in a `.Renviron` file or on the command line.)

    See `?Startup` for more details of environment files.

1. For all applications via Windows.  How you set an environment variable
...
strings, so for example one needs '`r xgobi`'.  You can make
...
and using the function `shortPathName` from R code.
...
-`HKEY_LOCAL_MACHINE\Software\R-core\R\Current Version` contains the version number, currently `r x.y`.
- `HKEY_LOCAL_MACHINE\Software\R-core\R\[version]\InstallPath` (where `[version]` is currently `r x.y`) contains the path to the R home directory.
...
`Software\R-core\R32` or `Software\R-core\R64`, for 32- and
...
(a) Use the Windows internet functions by starting R with the flag
`--internet2` ([How do I install R for Windows?](#install-windows))  or
calling `setInternet2(TRUE)` or setting the environment variable
`R_WIN_INTERNET2` to a non-empty value when starting R.  These cause
R to use the OS's functions used by Internet Explorer, which may already
be configured for use with proxies.  Note that this does _not_ work
with proxies that need authentication.
...
faulty versions of '`msvcrt.dll`': we have installed a workaround
...
This is a _warning_ which indicates that R has taken action to
...
Some users have found that `Rgui.exe` fails to start, exiting with
...
## Does R run under Windows Vista/7/8/Server 2008? {#Vista}
...
into your own file area.  (The same issues apply to the `.msi`
...
To do so, use the security tab on the '`Properties`' of the top-level
...
install R.
    ...
    make use of a 'personal library': [I don't have permission to write
    to the `r rw.lib` directory](#no-write-permission).
    ...
    all users via `R_LIBS_SITE` in `etc\Renviron.site`.  [What are HOME and working directories?](#what-home).  There is a standard location for a
...
permissions no longer work reliably.  `file.access` was re-written
...
default is `FALSE` except for the `Rgui` console.  There are
two potential problems with rendering directional quotes.  The first is
with running `Rterm`: in European locales the 'Windows Command
...
default font in the `Rgui` console and internal pager is
`Courier New`, which has directional quotes on all the systems we
tried.  `Lucida Console` which has elegant glyphs for directional
quotes (but seems rather light unless ClearType is in use):
`Consolas` is another font which we often select when ClearType is
in use.  Non-TrueType fonts such as `Courier` and `FixedSys`
lack directional double quotes on the systems we tried.

There is a related problem with using `Sweave` output in
`Rgui`, for LaTeX needs to be told about the encoding of
...
layouts, `1` in Eastern Europe, `[` in Portuguese, `4` or
`5` in Spanish, `/` in Francophone Belgian, and so on.  For
some keyboards the uses of AltGr are shown at
...
## Can I use R on 64-bit Windows? {#64windows}

The 32-bit build of R for Windows will run on both 32-bit and
64-bit[^foot32vs64] versions of Windows.  64-bit
...
[^foot32vs64]: what Windows calls x64 for x86-64 CPUs, not the very rare ia64 Windows for Itanium CPUs.
...
using the 32-bit build, replace '`x64`' by '`i386`' in the
...
example 32/64-bit `RODBC` need 32/64-bit ODBC drivers respectively,
...
drivers and so need to use 64-bit `RODBC` and hence R.  (And
...
and replace '`x64`' by '`i386`' or _vice versa_.
...
https://stat.ethz.ch/pipermail/r-help/2011-October/293413.html
...
## I want R in English (and not in French/Chinese/...)!

The default behaviour of R is to try to run in the language you run
Windows in.

Apparently some users want[^footlang] Windows in their native language, but not R.  To do so, set
`LANGUAGE=en` as discussed in Q2.2 and Q2.15, or in the
`Rconsole` file.

[^footlang]: or they may have no choice: apparently some Windows editions are tied to a specific language.
...
in files `Rconsole` and `Rdevga` (see `?Rconsole` or the
comments in the files: the system versions are in the `etc`
folder); in the latter you can replace `Arial` by `Arial
Unicode MS`, and we tried `FixedSys` and `MS Mincho` in
...
beware that `"Chinese"` is Traditional Chinese (code page 950,
Big5) and `"chs"` is needed for Simplified Chinese (code page 936,
GB2312).

When using `Rterm` the window in which it is run has to be set
up to use a suitable font (e.g. `Lucida Console` or
`Consolas`, not the OEM raster fonts) and a suitable codepage
(which for the Windows Cmd shell can be done using `chcp`).

## I selected English for installation but R runs in Chinese.

Precisely, you selected English **for installation**!  The language
...
messages by appending `LANGUAGE=en` to the shortcut you use to
...
`?Japanese` and `demo(Japanese)`) can be used on any graphics
device to display Japanese characters.

To use non-Latin-1 characters in the `postscript` graphics device,
see its help page (which also applies to `pdf`).

## I don't see characters with accents at the R console, for example in ?text.

You need to specify a font in `Rconsole` (see Q5.2) that supports
...
the `LANGUAGE` environment variable does not change the Windows
...
[//]: # (For further details see http://msdn.microsoft.com/library/default.asp?url=/library/en-us/intl/nls_0ddl.asp.
)
...
`.zip` file, or the package is a simple one involving no compiled
...
CRAN or from a local `.zip` file by using `install.packages`:
see its help page.  There are menu items on the `Packages` menu to
...
so for R 3.1.? the files are in `bin/windows/contrib/3.1`.

Note that the binary versions on CRAN are unsupported: see
http://cran.r-project.org/bin/windows/contrib/3.1/ReadMe,
...
`install.packages(type = "source")` or`R CMD INSTALL
...
so, just run `R CMD INSTALL pkgname` at a Windows command
...
pkgname`: see the _'Writing R Extensions'_ manual.
...
the instructions **exactly**.  At least 90% of the questions asked
...
## I don't have permission to write to the `r rw.lib` directory. {#no-write-permission}

You can install packages anywhere and use the environment variable
`R_LIBS` ([How do I set environment variables?](#set-environment)) to point to
the library location(s).

Suppose your packages are installed in `p:\myRlib`.  Then you can
EITHER
...
by running `Sys.getenv("R_LIBS_USER")`.  This will only be used if
it exists so you may need to create it: you can use

    dir.create(Sys.getenv("R_LIBS_USER"), recursive = TRUE)

to do so.  If you use `install.packages` and do not have permission
...
[Does R run under Windows Vista?](#Vista).  In particular, the detection
...
software and have its DLLs in your `PATH`.  Windows will normally
give an informative message about a certain DLL not being found.  See
http://cran.r-project.org/bin/windows/contrib/3.1/ReadMe for a
listing of some of these packages (notably `RGtk2`,
`cairoDevice`, `rggobi`, `rJava`, `rjags` and some
...
8.5.x installation with the path to its `bin` directory given by
value of `MY_TCLTK`, and that this is set up correctly (with
`TCL_LIBRARY` set if needed).  Note that you do need 8.5.x and not
...
(http://www.activestate.com/Products/activetcl/) as a way to
get Tcl/Tk extensions (but the support files do contain the most
commonly used `TkTable` and `BWidget` extensions).  This could
...
If you set `R_DEFAULT_PACKAGES` to include `foo`, you will
...
as shown in the `Select repositories...` item on the
`Packages` menu?

This reads from the tab-delimited file `R_HOME\etc\repositories`,
which you can edit, or put a modified copy at `.R\repositories`
in your HOME directory ([What are HOME and working directories?](#what-home)).
...
`MkRules.local` (after copying `MkRules.dist` to
`MkRules.local` if that has not already been done).
...
If you have a source package that is known to work on a Unix-alike
system, you can try the automated Windows binary package builder
documented at http://win-builder.r-project.org.  If the package is
...
software (often mentioned in the _ReadMe_ file in the binary
repository directory).  You can email R-windows@r-project.org
...
page at http://cran.r-project.org/mirrors.html.)
...
Packages without compiled code nor a `configure.win` script will
run on both 32- and 64-bit R.

Packages with compiled code but no `configure.win` nor
`src/Makefile.win` file will be built for both when running on a
...
other with option `--merge-multiarch`, but the package source
must be a tarball (and as before, running on a 64-bit version of Windows
with both versions of R installed).

Finally, a package without a `src/Makefile.win` file and no or
empty or architecture-independent `configure.win` file can be
...
- R commands can be interrupted by `Esc` in `Rgui.exe`
...
- Command-line editing is always available, but is somewhat simpler than the
    ...
    `README.rterm`.
- Paths to files (e.g.: in `source()`) can be specified with
    either "/" or "\\".
...
You have read the file `r readme.ver`? There are file menus on
...
If you resize the R console the `options(width=)` is automatically
set to the console width (unless disabled in the configuration file).

The graphics has a history mechanism.  As `r readme.ver` says:
...
There is a GUI preferences editor invoked from the `Edit` menu which
...
`windows` or as options: see `?windows`.  (Typically these are
of the order of 100.)
...
'`identify`' or '`readline`'.  Use the function
`'bringToTop()`'.  With its default argument it brings the active
...
paths.[^footcompleting]  Otherwise, it is completing R expressions: most obviously
...
[^footcompleting]: It does not have a complete understanding of Windows
...
`FALSE` turns it off completely for both `Rgui` and
...
`readline`-based command line interface on Unix-alikes: the OS X
GUI `R.app` has a different completion scheme.
...
## Can I load workspaces saved under Unix/GNU-Linux or Mac OS X?
...
non-ASCII character data may not be interchangeable even on
...
## When using Rgui the output to the console seems to be delayed. {#output-delayed}
...
option.  A call to the R function `flush.console()` will write out
the buffer and so update the console.


## Long lines in the console or pager are truncated.
...
http://www.netlib.org/blas/faq.html) routines are used to
speed up numerical linear algebra.  There is support in the R sources
for the 'tuned' BLAS called ATLAS
(http://math-atlas.sourceforge.net).  The savings can be
...
`R_HOME\bin\x64\Rblas.dll`, and you can add a fast BLAS just by
replacing that.  Replacements for 32-bit R and some of the older common
chips are available on CRAN in directory
`bin/windows/contrib/ATLAS`.  See the R Installation and
Administration' manual for how to build an ATLAS `Rblas.dll` tuned
...
http://en.wikipedia.org/wiki/Kazushige_Goto) for 64-bit Windows
by Ei-Ji Nakama can be found at
http://prs.ism.ac.jp/~nakama/SurviveGotoBLAS2/binary/windows/x64/.
Just download the file `Rblas.dll` appropriate to your CPU and
replace `R_HOME/bin/x64/Rblas.dll`.  (There is also a generic
version called '`DYNAMIC_ARCH`' that tries to adapt itself to the CPU
...
We strongly encourage you to do this _via_ building an R package:
see the _'Writing R Extensions'_ manual.  In any event you should
...
further options, or see `?SHLIB`.  (Replace `x64` by
`i386` for 64-bit R.)
...
not work at all.  If all you need is a _just-in-time_ debugger to
catch crashes, consider (32-bit) `Dr. Mingw` from the
`mingw-utils` bundle on http://www.mingw.org.  That will be
...
compiler.  Then you can debug by
    ...
    gdb /path/to/`r rw.ver`/bin/x64/Rgui.exe

(or use `Rterm.exe`.)  However, note

- `gdb` may only be able to find the source code if we run in the
    location where the source was compiled (`r gnu32` for
    the main system, `r src` for a package),
    ...
    in the file `.gdbinit` in the directory from which `gdb` is
    run.
- It is only possible to set breakpoints in a DLL after the DLL has
    ...
    package `stats` might be
         ...
         ...
    Alternatively, in `Rgui` you can use the 'Misc|Break to
...
(b) Include the C++ libraries in the link to make the DLL.  Suppose
    ...
    as in the example in _'Writing R Extensions'_.  Then build the DLL by
    (`gcc`)
...
##  The output from my C code disappears.  Why?

The `Rgui.exe` console is a Windows application: writing to
`stdout` or `stderr` will not produce output in the
console.  (This will work with `Rterm.exe`.)  Use `Rprintf` or
`REprintf` instead.  These are declared in header file
`R_ext/PrtUtil.h`.

Note that output from the console is delayed ([The output to the
console seems to be delayed](#output-delayed)), so that you will not normally see any
output before returning to the R prompt.


##  The output from my Fortran code disappears.  Why?

Writing to Fortran output writes to a file, not the `Rgui` console.
Use one of the subroutines `dblepr`, `intpr` or `realpr`
documented in the _'Writing R Extensions'_ manual.
...
call `R_ProcessEvents()` periodically from your compiled code.
If you want output to be updated on the console, call
`R_FlushConsole()` and then `R_ProcessEvents()`.

Last edited `r last.edit`: comments to R-windows@R-project.org

Converted from texi format into RMarkdown format by Thierry Onkelinx.
